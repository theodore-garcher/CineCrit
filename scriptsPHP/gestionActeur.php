        ...
        $count = $prep->rowCount();
            ...
            if ($isAdmin && !empty($_POST) && isset($_POST["idperso"])) {
                if (isset($_POST["action"]) &&  $_POST["action"] == "Retirer") {
                    $prep = $cnx->prepare("DELETE FROM cinecrit.jouer WHERE idfilm = ? AND idpersonalite = ?");
                    $prep->execute([$_GET["idfilm"], $_POST["idperso"]]);

                } else if (isset($_POST["action"]) &&  $_POST["action"] == "Ajouter") {
                    $prep = $cnx->prepare("INSERT INTO cinecrit.jouer (idfilm, idpersonalite) VALUES (? , ?)");
                    $prep->execute([$_GET["idfilm"], $_POST["idperso"]]);
                }
            }
            ...
            echo "<h1>Acteurs présents dans ". $titrefilm ."</h1>";
            ?>
            <div class="catalog">
                <table>
                <tr>
                ...
                <?php
                    $prep = $cnx->prepare("SELECT * FROM cinecrit.personnalite NATURAL JOIN cinecrit.jouer NATURAL JOIN cinecrit.film WHERE cinecrit.film.idfilm = ?");
                    $data = $prep->execute(array($_GET['idfilm']))->fetchAll();
                    $count = $data->rowCount();
                    ...
                    if ($count > 0) {
                        ...
                        echo "<tr><td colspan=4><p>Aucun acteur n'a été enregistré.</p></td></tr>";
                    }
            ...
            <?php
            if ($isAdmin) {
                echo "<h1>Ajouter des acteurs</h1>";
                echo "<h2>Rechercher un acteur</h2>";
                ...
                <div class="catalog">
                    ...
                    <?php
                        ...
                        if (isset($_POST['searchDemandPerso']) == false){
                            $prep = $cnx->prepare("SELECT * FROM cinecrit.personnalite WHERE cinecrit.personnalite.idpersonalite NOT IN (SELECT cinecrit.personnalite.idpersonalite FROM cinecrit.personnalite JOIN cinecrit.jouer ON (cinecrit.personnalite.idpersonalite = cinecrit.jouer.idpersonalite AND cinecrit.jouer.idfilm = ?))");
                        ...
                        foreach ($data as $row) {
                            ...
                            echo "<td>". $row["datenaissance"] ."</td>";
                            echo "<td>". $row["nationaliteperso"] ."</td>";
                            echo "<td><form method=\"POST\" action=\"gestionActeur.php?idfilm=". $_GET['idfilm'] ."\">";
                            echo "<input type=\"hidden\" name=\"idperso\" value=\"". $row["idpersonalite"] ."\"/>";
